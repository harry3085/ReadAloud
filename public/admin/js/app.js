...
const _SPEAK_EVENT_LABELS = {
  ...
  'warn:samsung':     { label: '삼성 인터넷 진입',   color: '#d97706' },
};
  ...
  if (q.type === 'sentence') {
    ...
    return `<div style="border:1px solid var(--border);border-radius:6px;padding:12px;margin-bottom:8px;">
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:6px;">
        ...
        ${chunks.length > 0 ? `
        <div style="padding:8px 12px;background:#f0fdfa;border-radius:6px;border-left:3px solid #0d9488;">
          <div style="font-size:10px;color:#0f766e;margin-bottom:3px;">청크 학습용 (${chunks.length}개)</div>
          <div style="display:flex;gap:4px;flex-wrap:wrap;">
            ${chunks.map(c => `<span style="padding:3px 8px;background:white;border:1px solid #99f6e4;border-radius:4px;font-size:12px;color:#0f766e;">${esc(c)}</span>`).join('')}
          </div>
        </div>` : ''}
      </div>
    </div>`;
...
function _qsRenderEditQuestion(q, idx) {
  ...
  // 2026-07-22 문장시험 편집 (ko + en + 청크 학습용 chunkedEn optional)
  if (q.type === 'sentence') {
    const wc = q.wordCount || (q.en || '').split(/\s+/).filter(Boolean).length;
    const chunkedEn = q.chunkedEn || '';
    const chunks = chunkedEn.split('/').map(s => s.trim()).filter(Boolean);
          ...
          ? `<div style="margin-bottom:14px;padding:10px 12px;background:#eff6ff;border-radius:6px;border:1px solid #bfdbfe;">
              ...
              <div style="display:flex;gap:16px;margin-top:8px;padding-top:8px;border-top:1px dashed #bfdbfe;flex-wrap:wrap;">
                ...
                <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text);cursor:pointer;" title="불통 학생 재응시 시 이전에 틀린 문제만 다시 풀게 함">
                  <input type="checkbox" id="tpVocabRetryWrongOnly" checked onchange="document.getElementById('tpVocabRequirePerfect').disabled=!this.checked;if(!this.checked)document.getElementById('tpVocabRequirePerfect').checked=false;"> 틀린문제만재응시
                </label>
                <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text);cursor:pointer;" title="100점 될 때까지 재응시 반복 (틀린문제만재응시 선택 시 활성화)">
                  <input type="checkbox" id="tpVocabRequirePerfect" checked> 100점까지
                </label>
            ...
            if (c) {
              ...
              if (tMode === 'recording') {
                ...
                if (isSubmittedWithRecs) {
                  ...
                  recs.forEach(r => {
                    // 카드 라벨 = 항목명만 (수치 없이) — 학원장 요청 2026-06-30
                    if (r.voiceActivity != null && r.voiceActivity < 0.10) abnormalReasons.push('음성');
                    if (r.voiceBandRatio != null && r.voiceBandRatio < 0.30) abnormalReasons.push('명료도');
                    if (r.monotony != null && r.monotony > 0.70) abnormalReasons.push('단조');
                    if (minDur > 0 && r.duration != null && r.duration < minDur * 0.5) abnormalReasons.push('짧음');
                    // 시간 비율 — < 90% = 강한 빨강 (90~110% 또는 > 110% = 정상)
                  ...
                  // 학원장 정책 — AI 점수보다 객관 측정(완독·도달) 우선 (2026-06-30 도달 100% → 90% 완화)
                  const hasNormalIndicator = recs.some(r =>
                    (typeof r.lastReadPosition === 'number' && r.lastReadPosition >= 90)
