// 메시지 첨부 파일 10일 자동 삭제 GCS Lifecycle Rule 설정
//
// messageAttachments/* 경로의 10일 이상 객체 자동 삭제
// 학원 무관 일괄 적용 (정책 통일)
//
// 사용:
//   node scripts/admin/set-message-attachments-lifecycle.js          (DRY-RUN)
//   node scripts/admin/set-message-attachments-lifecycle.js --apply  (실제 적용)
...
const NEW_RULE = {
  ...
  condition: {
    age: 10,
    matchesPrefix: ['messageAttachments/'],
  },
};
...
(async () => {
  ...
  const hasRule = currentRules.some(r =>
    ...
    r.condition?.matchesPrefix?.some(p => p === 'messageAttachments/' || p === 'messageAttachments')
  );
  ...
  if (hasRule) {
    console.log(`\n이미 messageAttachments/ 규칙이 있습니다 — 변경 없음.`);
    process.exit(0);
  }
  ...
  console.log(`\nLifecycle 규칙 적용 완료`);
  console.log(`  messageAttachments/* 의 10일 이상 객체는 GCS 가 매일 자동 삭제합니다.`);
