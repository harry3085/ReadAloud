...
## 2026-08-19: 시험 펼침 카드에 출제 옵션 요약 라인

SW v711 → v712 (3 commit `fe1df08`·`98b97f3`·`afc4cc3`). 학원장 질문 —
"출제된 시험의 옵션이 어떻게 되는지 어디서 확인할 수 있지?" 진단 결과
현재 부분(형식/객%) 만 진도체크 일자별에 표시되고 그 외엔 없음.

### 신규 헬퍼 `_tpBuildOptionsLine(t)` (`fe1df08`)

시험 유형별 옵션 요약. `tpToggleTestProgress` 펼침 카드 상단에 삽입 —
시험관리 · 시험목록 · 진도체크 3경로 공통 (같은 헬퍼 사용).

- **vocab**: 형식(말하기/객주비율/영한비율/객→주순 등) + [틀린문제만] [100점까지]
  + 통과점수 + 제한시간 + 엄격도(말하기)
- **sentence**: 매칭 임계 + 힌트 허용 + 통과점수
- **recording**: 최소/최대 시간 + 응시 횟수 + 통과점수
- **공통**: 통과 N점, 제한시간 (있으면)

### 컴팩트 스타일 (`98b97f3`)
학원장 후속 — 배지형(padding·배경색) 이 화면 많이 차지.
- padding 8px 12px 6px → 3px 12px (높이 절반)
- 배경색 배지 (border-radius:10px) → 인라인 텍스트 + · 구분자
- 색상 강조만 유지 (틀린문제만=보라 / 100점까지=빨강 / 통과=초록 / 말하기=빨강)

예: `출제 옵션  객 50% / 주 50% · 영→한 50% · 틀린문제만 재응시 · 100점까지 · 통과 80점 · ⏱ 30초/문`

### SW bump 누락 fix (`afc4cc3`)
fe1df08 + 98b97f3 커밋 시 SW bump 잊음. 학원장 강력 새로고침해도 옛 캐시
가능 → v712 로 bump. **관련 작업 규칙 재확인 필요** — admin/js/app.js
변경도 SW bump 필수 (student js 만이 아니라 학원장 앱도 SW 캐시 대상).

### 파일 크기 / SW 캐시 (2026-08-19)
...
3. 문장시험 학습 3단계 실사용 관찰 / timing 로그 제거 (변동 없음)
4. Phase 5 출시 준비 (변동 없음)

※ 인쇄 출제 문제수는 현행 유지 확정 (학원장 결정 2026-09-18) — 그냥 인쇄 = 앞에서부터 N개(순서대로),
  🔀 문제 섞기 후 인쇄 = 랜덤 N개. 두 방식 모두 선택 가능하므로 자동 랜덤으로 바꾸지 않음.

---

## 2026-09-18 (이어서): 아이폰 말하기 원인 확정 + 전 유형 중간 저장·이어풀기 + 공지 고정 + 학습 횟수 설정

SW v802 → v811 (9 commit). 학원장이 학생 폰(임하율·박선후)을 직접 보며 원인 확정 → 안내·기능 반영.

### 1) 아이폰 말하기 실패 원인 2가지 확정 (speakLog 로 추적)
- **임하율 `service-not-allowed`** = 설정 → 개인정보 보호 및 보안 → **음성 인식 → Safari 꺼짐**.
  받아쓰기 활성화·마이크 권한과 **별개 권한**. 켜니 즉시 정상 (v804: 안내 1순위로)
- **박선후 "읽어주기 소리 안 들림"** = **아이폰 무음 모드** (Safari TTS 음소거). 소리가 안 나니 학생이
  말을 안 함 → 3회 연속 무음 알림 → "마이크 확인" 문구가 원인을 오도. v805~806: 무음 해제 안내 추가 +
  3회 무음 알림을 "🔊 읽어주는 소리가 들리나요?" (소리 → 마이크 순) 로 재작성
- 기록의 `Mac · Safari` = 아이폰 Safari **데스크탑 웹사이트 요청 모드** (아이패드 아님). `_isIos()` 는
  MacIntel+터치로 판정해 동작 영향 없음, speakLog 라벨만 Mac (보정은 미착수)
- iOS 27.0 학생(박선후·차윤민) 무응답 반복 — 박선후는 무음 모드였음, 차윤민 미확인

### 2) 중간 저장·이어풀기 — 전 유형 (v807~808)
단어시험(`_vqSaveProgress`) 정책을 모든 유형에 동일 적용: 중단 시(진행분 있을 때) 저장 여부 질문 →
재진입 시 "N번부터 이어서?" (아니오=처음부터) → 제출 시 삭제. **localStorage, 당일(KST) TTL**.
- 학습(단어 따라읽기·문장 청크): `_vpSaveProgress/_vpLoadProgress/_vpTryResume`, 런타임 필드 제외 저장(`_VP_PROG_KEYS`)
- 객관식·빈칸·언스크램블·문장매칭: 공용 `_exProgSave/_exProgLoad/_exProgClear/_exProgAskResume/_exQuitSave`
  (`exProgress_{kind}_{testId}_{uid}`). 셔플 순서(선지·청크·playOrder) 그대로 복원
- 녹음숙제는 기존 서버 자동 저장(inProgress) 유지. 해석하기는 학생앱 응시 화면 없음

### 3) 중간 저장 학원장 표시 (v809)
- 학생앱 `_syncSavedProg` → `users/{uid}.savedProgress.{testId} = {done,total,ymd,savedAt}` (본인 doc, Rules 무변경).
  로컬 저장분이 실제 있었을 때만 deleteField (불필요 write 방지)
- **userCompleted 에 박지 않은 이유**: 점수 없는 doc 이 시험 펼침 카드·doneCount 등에서 응시(0점)로 오인됨
- 학원장 `_savedProgLine`: 오늘 저장분만 `⏸ 8/20 저장 · 19:32` — 시험 펼침 카드(대기·미통과) + 학생별 진도체크
  (학생별은 users doc 1 read 추가)

### 4) 공지 정비 + 고정 기능 (v810)
- 3번 공지 → "▣ 말하기 학습·시험(단어·문장) 안 될 때" 로 교체 (녹음숙제와 분리, 간결화, 홈화면 아이콘 오안내 제거).
  1번(녹음숙제 마이크) 공지에 말하기 공지 안내 + "말하기는 Safari 에서만" 반영. 원본 백업은 scratchpad
- 공지 고정: `notices.pinned/pinnedAt`, `_noticePinSort` (고정 먼저·고정시각 최신순 + 나머지 createdAt desc).
  학원장 공지관리 제목 앞 핀 토글(surgical), 학생앱 홈·목록 + 대시보드 핀 표시. ICONS `pin` 양쪽 추가.
  말하기 공지는 고정해 둠

### 5) 단어 학습 말하기 횟수 설정 (v811)
- 출제 모달 형식=단어 학습 시 `[말하기 횟수 N]` (1~5, 기본 2) → `vocabOptions.practiceCount`
- 학생앱 customMin=N, customMax=N+1 — **N번째 시도가 Great** 면 다음 단어, 아니면 한 번 더. 옛 시험은 기존 2/3
- 문장 청크 학습은 기존 1/2 유지

### 작업 규칙 추가 (2026-09-18 이어서)
- **아이폰 말하기 점검 순서**: Safari 접속(홈화면 아이콘 X) → 무음 모드 → 개인정보 보호 및 보안 > 음성 인식 > Safari
  → 받아쓰기 → 스크린타임 Siri 및 받아쓰기. `service-not-allowed` 는 받아쓰기가 켜져 있어도 Safari 음성 인식 권한일 수 있음
- **웹은 기기 무음·출력 여부를 감지 못함** — 무음 모드는 사전 차단 불가, 증상(무음 누적) 시점 안내로 대응
- **알림 문구는 실제 원인 순서로** — "3회 무음"의 최다 원인은 소리 안 들림이었는데 "마이크 확인"으로 안내해 오도.
  증상 → 가장 흔한 원인부터 나열
- **점수 없는 보조 정보는 userCompleted 에 넣지 않기** — doc 존재·필드 유무로 응시 판정하는 화면이 많음.
  별도 위치(users 맵 등)에 저장
- **운영 데이터(공지 등) 직접 수정 시 원본 백업 후 반영**, 학생에게 즉시 노출되므로 초안 확인 후 진행
- **여러 줄 치환 패치는 파일로 작성한 Python 스크립트로** — bash heredoc 인용이 깨져 일부만 적용(v803 SW 만 bump)
  된 사례. assert 로 매칭 1건 확인

### 파일 / SW 캐시 (2026-09-18 이어서)
- `public/js/app.js`: 안내 문구 3곳 + 학습·시험 중간 저장/이어풀기 + savedProgress 동기 + 공지 고정 정렬
- `public/admin/js/app.js`: 중간 저장 표시 + 공지 핀 토글 + 말하기 횟수 옵션
- Firestore: notices 3번 교체·고정, 1번 문구 수정
- SW 캐시: `kunsori-v802` → `kunsori-v811`

**다음 세션 후보**:
1. 중간 저장·이어풀기 실사용 관찰 (저장 유효기간 당일 유지 여부 — 학원장 미결정)
2. 박시하·차윤민 무응답 반복 — 무음 모드·Safari 음성 인식 권한 확인 요청
3. speakLog 데스크탑 모드(Mac 표시) 라벨 보정 — 학원장 요청 시
4. 단어 학습 Great 판정 기준 (N번째 시도 vs 누적 최고) — 학원장 요청 시
5. Phase 5 출시 준비 (변동 없음)

---

## 2026-09-21: 단어시험 '듣고 선택하기' 신규 형식 + 객관식 보기 풀 fix + 영문 문장부호 허용

SW v811 → v816 (5 commit). 학원장 요청으로 신규 시험 형식 추가 → 그 흐름에서
객관식 보기 생성 버그 발견·수정 → OCR 단어장 저장 차단 보고로 문자 게이트 완화.

### 1) 듣고 선택하기 (listening) — 단어시험 신규 형식 (`8ebd65c`, v812)

TTS 가 영단어를 읽어주고 학생이 **한글 뜻 4지선다**로 고르는 형식. 단어는 화면에
숨기고 음성으로만 제공.

**데이터 모델** — 새 testMode 가 아니라 `vocabOptions.format = 'listening'`
(단어시험 배정 인프라 전부 재사용). 학생앱이 문제별로 `{format:'mcq',
direction:'en2ko', listening:true}` 강제 배정 → 채점·저장·상세 경로는 기존 MCQ 그대로.

**학원장**
- 출제 모달 형식 select 에 `듣고 선택하기 (음성 → 4지선다)` 추가 — **학습(따라 읽기)
  위**에 배치 (시험 그룹). 사용자 요청: "이건 시험이니까 학습 위쪽으로"
- 형식·방향 고정이라 객관식비율·영→한비율 슬라이더만 자동 비활성
  (`_tpVocabFormatChanged` 의 `disableRatio` 에 isListening 추가)
- 보기섞기 · 문제섞기 · 틀린문제만재응시 · 100점까지 · 통과점수 · 제한시간은
  **다른 시험과 완전 동일** (사용자 명시 요구)
- 옵션 요약 라인 `🔊 듣고 선택하기` / 성적 상세 라벨 `🔊 듣고 고르기 · 힌트 사용`
- 말하기 전용 게이트(`_tpSpeakingUnfitGate`)·homophones 채움은 타지 않음

**학생앱**
- 문제 진입 시 단어 숨김 + TTS 1회 자동 재생
- `[🔊 다시 듣기]` 무제한 / `[💡 힌트 (단어 보기)]` — 누르면 단어 공개, **감점 없음**
  (`ans.hintUsed` 로 사용 여부만 기록 → 학생·학원장 상세에 표시)
- 채점 시점에 단어 자동 공개 (정답 뜻과 같이 확인)
- 홈 시험 카드 `🔊 듣고 고르기` 배지
- 중간 저장·이어풀기에도 숨김·힌트 상태 보존 (answers 에 박히므로 자동)

### 2) 객관식 보기가 '—' 로 비던 버그 (`d5be3d4`, v813)

학원장 보고 — "틀린문제 다시풀기 할 때 4개 지문이 없는 것이 있음". 원인 **2개**:

- **보기 풀이 재응시 목록으로 축소** — 오답 3개를 `questions`(틀린문제만 남은 배열)
  에서 뽑아, 남은 문제가 1~2개면 `—` 로 채워짐.
  → 재응시 필터 **전**의 전체 목록을 `choicePool` 로 보관해 거기서 추출
- **거르는 순서가 거꾸로** — `_rngShuffle(pool).slice(0,3).map().filter(x && x!==correct)`
  라 3개를 먼저 자른 뒤 걸러서 모자람. 일반 응시에서도 뜻이 비었거나 정답과 같은
  단어가 섞이면 `—` 발생.
  → 3개가 **찰 때까지** 훑으며 빈값·정답중복·보기중복 제외 (`seen` Set)

이제 `—` 는 시험 단어 자체가 4개 미만일 때만 나옴. 객관식 본문이해·문법은 AI 출제
시점에 보기 4개가 박혀 저장되므로 해당 없음 (런타임 생성 X).

### 3) 듣고 선택하기 — 오디오 인디케이터 (`69ff3ad`, v814)

학원장 요청 — "단어 나오는 자리에 말하기 학습에 쓰이는 오디오 리액션을 넣을 수 있나?"

- `🔊 ? ? ?` 텍스트 폐기 → 말하기 학습의 TTS 인디케이터 5종(ribbon/pill/band/blob/flow)
  재사용. 시험 진입 시 1종 랜덤 (`_vqState.vizType`), 세션 내 고정
- **빌더 5종에 색 팔레트 인자 추가** (`_vpBuildXHtml(c = _VP_COLORS)`) + 디스패처
  `_vpVizHtml(type, c)` 신설. 코랄 헤더 위라 듣기용은 흰색 팔레트
  (`_VQ_LISTEN_COLORS`). 학습 화면은 기존 코랄 그대로 (기본값)
- `.vq-viz` CSS — 66px 컴팩트 슬롯. **재생 중만 또렷(opacity 1), 대기 중 흐리게(.38)**
  이지만 자리는 항상 유지 → 카드 높이 안 흔들림
- `_vqSpeakListenWord` — TTS onend/onerror 에 인디케이터 off + 6초 안전망 타이머.
  Chrome `cancel()` 직후 speak 시 첫 음절 잘림 대비 80~130ms 여유 (듣기는 음성이 전부)

### 4) 정답 공개 시 화면 밀림 제거 (`e8546f5`, v815)

단어를 `display:none` 으로 숨기다 보니 힌트·채점으로 단어가 뜨면 카드가 한 줄 늘어
아래 보기·푸터가 밀림. → 듣기 모드에서는 단어 영역을 **항상 표시하되 내용만 비우고**
`min-height:29px`(22px × line-height 1.3) 로 한 줄 자리 선점. 다른 형식 전환 시 원복.

### 5) 단어 세트 영문 문장부호 허용 + 스펠 자동 처리 (`2af2d37`, v816)

학원장 보고 — OCR 단어장(Nate the Great CH2) 저장 시 오류. 진단 결과 문자 게이트
(`_qsValidateWordChars`)가 24개 중 3개 차단: `so what?`(?) · `but = except`(=) ·
`He does do good work!`(!).

부수 확인 — "느낌표는 되는데 물음표는 안 된다"는 학원장 인상은 **코드상 근거 없음**
(`!` `?` 동일 규칙 한 줄). OCR 이 문장 끝 `!` 를 놓쳤을 가능성이 큼.

**차단 이유가 이미 해소돼 있었음** (2026-05-24 도입 근거 = "학생이 특수문자를 못 쳐서
제출 불가"):
- `_vqNormStr` 이 채점 시 `[^\p{L}\p{N}\s]` 제거 → 스펠링은 문장부호 무시 비교
- `_spkNorm` 도 `[^a-z0-9\s]` 제거 → 말하기도 무관
- 제출 버튼은 글자수 일치가 아니라 "입력 비어있지 않음"만 검사

**수정**
- 게이트 허용 문자에 `? ! = : ; " ( )` 추가. 한글·한자·일본어는 계속 차단
  (모바일 IME 로 영단어 칸 입력 자체가 막히는 진짜 문제)
- `_vqAutoSpaces` 일반화 — 공백만 자동 삽입하던 것을 **공백+문장부호** 로
  (`'sowhat'` + `'so what?'` → `'so what?'`). 공용 판별 `_VQ_NON_LETTER` 3종 신설
- Backspace 핸들러 — 자동 삽입된 문장부호를 건너뛰어 앞 글자까지 삭제 (연속 기호 대응)
- 스펠 칸 표시 — 공백·문장부호는 채울 네모 대신 **고정 표시** (`s o ␣ w h a t ?`).
  다단어 정답의 공백도 네모 대신 여백으로 바뀜 (기존 동작 변경)
- 힌트 `N글자` 도 문장부호 제외 계산
- 말하기 출제 게이트(`_tpSpeakingUnfitReasons`)는 그대로 — `=` `/` `(` `)` 등
  발음 불가 표기는 계속 배정 단계에서 차단, `so what?` 은 통과

### 작업 규칙 추가 (2026-09-21)

- **런타임 생성 보기(distractor)의 풀은 필터 전 전체 목록을 따로 보관** — 틀린문제만
  재응시처럼 문제 배열 자체가 줄어드는 옵션이 붙으면, 그 배열을 보기 풀로 쓰던 코드가
  조용히 망가짐(빈 보기). 필터링과 풀 확보 시점을 분리.
- **"N개 뽑고 거른다" 는 항상 "거르며 N개가 찰 때까지" 로** — `slice(0,N).filter(...)`
  패턴은 걸러진 만큼 모자람. 순서를 뒤집어야 항상 N개 보장.
- **제약(게이트)을 넣었으면 그 근거가 아직 유효한지 주기적으로 재확인** — 문자 게이트는
  "학생이 특수문자를 못 친다"가 근거였는데, 그 사이 채점이 문장부호를 무시하도록
  바뀌어 근거가 소멸. 학원장이 불편을 보고할 때 게이트 완화가 정답인 경우가 있음.
- **학원장 인상(印象) 보고는 코드로 먼저 반증** — "느낌표는 되고 물음표는 안 된다" 를
  node 로 실제 문자 테이블을 돌려 둘 다 차단임을 확인 → OCR 누락이라는 진짜 원인에 도달.
  추측으로 비대칭 규칙을 찾아 헤매지 말 것.
- **공용 SVG 빌더는 색 팔레트를 인자로** — 같은 애니메이션을 배경색이 다른 화면에서
  재사용할 때 기본값 인자(`c = _VP_COLORS`)면 기존 호출부 무변경으로 확장 가능.

### 파일 / SW 캐시 (2026-09-21)
- `public/js/app.js`: listening 배정·렌더·힌트·재생, 보기 풀 fix, 오디오 인디케이터,
  문장부호 자동 채움·칸 표시·Backspace (+~180줄)
- `public/admin/js/app.js`: 형식 option·`_tpVocabFormatChanged`·옵션 요약·상세 라벨,
  문자 게이트 허용 확대
- `public/_app.html`: `#vqListenArea`(다시 듣기·힌트) + `#vqVizSlot` + `.vq-viz` CSS
- SW 캐시: `kunsori-v811` → `kunsori-v816`

**다음 세션 후보**:
1. **듣고 선택하기 실사용 피드백** (인디케이터 크기 66px·흐림 정도, TTS 속도 0.9배,
   제한시간 기본 30초 적정성, 정답 배너 자리 고정 여부)
2. **문장부호 허용 후 관찰** — 스펠 칸 고정 표시(공백 여백화) 에 대한 학생 반응
3. 문제세트 전체를 보기 풀로 쓰는 옵션 (현재는 그 시험에 출제된 단어 전체가 상한.
   `vocabOptions.distractorPool` 로 확장 가능 — 학원장 요청 시)
4. 듣고 선택하기에도 발음 불가 표기 게이트 적용 여부 (TTS 가 `rice/fall` 을
   "rice slash fall" 로 읽음 — 현재 미적용)
5. 중간 저장 유효기간 / speakLog 데스크탑 모드 라벨 / Phase 5 출시 준비 (변동 없음)

---

## 2026-09-22: 학생앱 "팅김" 근본 원인 3종 + 홈화면 설치 정책 + 일일 오류 리포트 자동화

SW v816 → v818 (2 commit). 학원장 "김수호 학생이 시험·학습 중 자주 팅긴다" 보고 →
speakLog 로 팅기는 순간 특정 → 코드 결함 2건 + 운영 요인 1건 수정 → 전 학생 스캔으로
같은 패턴 집단 확인 → 일일 자동 점검 체계 구축.

### 1) 진단 — 팅기는 순간이 로그에 그대로 남아 있었음

김수호 speakLog 19건 전수. 결정적 패턴:

```
9/17 19:46:52 enter → 19:47:45 sr-error network → 19:47:52 enter(7초 뒤)
     19:48:41 sr-error network → 19:48:50 enter → 19:49:23 sr-error network
     19:50:08 다른 폰으로 교체
```

3분간 3회 강제 퇴장 후 폰 교체. 기록 19건 전부 **단어 학습·문장 청크 학습**.

**기기**: 갤럭시 S21(Android 15·Chrome·홈화면앱) / 갤럭시 S25+(Android 16·삼성
인터넷·홈화면앱) **2대 번갈아 사용**. 9/21 21:30:38 재로그인은 팅김이 아니라 폰 교체.

**`network` 오류의 정체** — Chrome 음성 인식은 오디오를 구글 서버로 보내 텍스트를
받음. `network` 는 그 통신 1회 실패이며 **폰 인터넷 끊김과 다름**. 같은 시각 Firestore
읽기(학습 재진입)는 성공 → 인터넷은 정상이었음이 데이터로 확인됨.

### 2) 수정 3종 (`c983386`, v817)

**① 학습 중 일시적 오류 → 홈으로 강제 퇴장하던 것**
`_vpShowSrIssue` 가 모든 코드에서 `stopped=true` + `goHome()`. 일시적 오류까지
설정 문제와 동일 취급.
- `_VP_TRANSIENT_SR = {network, language-not-supported}` → 같은 자리에서
  **[다시 시도]** (하던 곳부터 이어서). 권한·설정 문제만 종전대로 홈으로
- 단어시험 말하기(6078)·문장시험(7943)은 원래 안내만 띄우고 계속 — **학습만 달랐음**

**② 학습 화면(`vocabPractice`)이 `_EXAM_SCREENS`·`_EXAM_QUIT_FNS` 에서 누락**
한 곳 누락으로 3가지가 동시에 깨져 있었음:
- **wake lock 미적용** ← 학습은 듣고 따라 말하는 흐름이라 터치가 없어 30초~1분이면
  화면이 꺼지고 SR·TTS 중단 (체감 팅김의 큰 부분으로 추정)
- SW 업데이트 시 학습 도중 즉시 새로고침 (시험은 끝날 때까지 대기)
- 뒤로가기 시 중단 확인·저장 모달 없이 이탈

**③ SW 자동 새로고침 시점 — 사용 중 금지로 변경**
최근 30일 배포 **102회** / 7일 20회. 옛 동작은 감지 즉시(시험 중이면 끝나자마자)
`location.reload()` → 홈화면 앱이라 스플래시까지 떠 재시작처럼 보임.
- `_pendingReload` 표시만 하고, **앱을 나갔다 돌아왔을 때**(visibilitychange
  hidden→visible) 적용. `show()` 훅 제거
- 배포가 잦아도 학생 흐름이 끊기지 않음

**④ 삼성 인터넷 진입 시 Chrome 권유** (세션당 1회) + 학원장 기록 라벨 `warn:samsung`

### 3) 전 학생 스캔 — 같은 패턴 집단 확인

`scripts/diag/scan-speaklog-crash.js` 신규. default 학원 87명 중 **기록 보유 70명**.

**강제 퇴장 40건 중 39건이 학습 유형** (vocab-practice 35 · sentence-chunk 4) —
위 ①이 고친 바로 그 경로. 단어시험은 1건.

| sr-error 원인 | 건수 | 처리 |
|---|---|---|
| `network` | 20 | v817 로 해결 (자리 재시도) |
| `not-allowed` (마이크 권한) | 15 | 학생 설정 조치 필요 |
| `service-not-allowed` (iOS 음성인식) | 6 | 학생 설정 조치 필요 |
| `audio-capture` | 1 | 학생 조치 |

**`block:no-sr` 81건 = 100% iOS/Mac Safari 홈화면앱** — 아이폰은 홈 화면 아이콘에서
음성 인식 불가(애플 제약). 해당 14명은 말하기를 **한 번도 시작 못 함** (유주아 18회
전부 차단). "팅김"이 아니라 "안 된다" 불만의 최대 원인.

**`sr-hang` 117건 중 110건이 iOS Safari** (안드로이드 7건) — WebKit Bug #321436.
박시하 26 · 박소율 19 · 범지원 13 · 차윤민 13 · 박선후 10.

**환경 위험군** — 삼성 인터넷 12명 / 카카오톡 인앱 4명(정하연·최건희·용주영·조민서) /
iOS 홈화면앱 14명.

### 4) 홈화면 추가 메뉴 — 안드로이드만 노출 (`28e210b`, v818)

⋮ 메뉴의 [홈화면에 추가] 가 **아이폰에도 떠서 설치를 유도**하고 있었음 — §3 의
`block:no-sr` 81건을 앱이 스스로 부추긴 셈.

- 노출 규칙: 숨김 = 이미 홈화면 앱 실행 중 **또는 아이폰·아이패드**(`_isIos()`),
  노출 = 안드로이드·PC
- ⋮ 버튼에 안내 점 — 설치 전에만, 한 번 눌러보면 사라짐(`localStorage.a2hsSeen`)
- 메뉴 열 때마다 재판정(`toggleDropdown`) + `appinstalled` 시 즉시 숨김
- 아이폰에서 호출되면 설치법 대신 "홈화면 아이콘 쓰지 말고 Safari 로 + 기존 아이콘
  삭제" 안내
- `alert()` 3곳 → 앱 표준 모달(`_showSrGuideModal`)로 교체 (작업규칙 2 위반분).
  안내용 `tone:'info'` (제목 브랜드색) 옵션 추가

### 5) 일일 오류 리포트 자동화

`scripts/diag/daily-error-report.js` 신규 — 최근 24시간 집계:
- **조치 필요 학생** (이름·반·원인·학원장이 전할 조치 한 줄) / `[신규]` 표시
- 조치 불필요(network·무음)는 분리 표기 → 실제 조치 대상만 눈에 들어오게
- 접속 환경 집계 + 녹음숙제 서버 오류

**이중 실행 체계**:
| | 주체 | 시각 | 결과 | 생존 |
|---|---|---|---|---|
| Windows 작업 스케줄러 | `ReadAloud-DailyErrorReport` | 08:45 | `scripts/diag/logs/error-YYYY-MM-DD.txt` | PC 켜져 있으면 영구 |
| Claude 세션 cron | 이 세션 | 08:47 | 읽고 **요약 보고** | 세션 유지 시 7일 |

**클라우드 루틴(/schedule)은 불가** — 클라우드 에이전트는 로컬 머신 접근 불가 →
`scripts/.firebase-admin-key.json`(git 미추적) 없음 + GitHub 미연결. 키를 클라우드에
올리는 건 학생 개인정보 전체 접근 권한 서비스 계정을 외부에 두는 것이라 보류.

### 6) 학부모·학생 카톡 안내문 2종 작성 (발송은 학원장)

- **갤럭시(삼성 인터넷) 10명**: 아이콘 삭제 → Chrome 직접 입력 접속 → Chrome 에서
  홈화면 추가
- **아이폰 14명**: 아이콘 삭제 → Safari 직접 입력 접속 → **다시 추가하지 말 것**
- 두 문구 모두 **"카톡에서 주소를 누르지 말고 직접 입력"** 경고 포함
  (카톡 링크 탭 → 카톡 인앱 브라우저 → 말하기 또 안 됨)
- 김다윤·김도윤은 갤럭시·아이폰 둘 다 사용 → 두 메시지 모두 대상

### 작업 규칙 추가 (2026-09-22)

- **홈화면 아이콘(PWA)은 만든 브라우저의 엔진을 그대로 쓴다** — 삼성 인터넷에서 만든
  아이콘은 영원히 삼성 엔진. 겉모습이 같아 학생은 구분 못 함. "Chrome 쓰세요" 안내만
  으로는 해결 안 되고 **기존 아이콘 삭제가 선행**돼야 함. 기기 기록의
  `삼성 브라우저 · 홈화면 앱` 은 번갈아 쓴다는 뜻이 아니라 한 번의 접속 속성 2개.
- **iOS 홈화면 PWA = 음성 인식 완전 차단** — 설치를 권하는 UI 자체를 노출하면 안 됨.
  PWA 설치 유도는 플랫폼별로 득실이 반대일 수 있으니 기능 의존성(SR·마이크)을 먼저 확인.
- **일시적 오류와 설정 오류는 처리 경로를 분리** — 일시적(network 등)인데 세션을
  종료시키면 사용자는 "팅겼다"로 인식하고 처음부터 재진입. 같은 오류라도 같은 화면
  유형끼리 처리가 갈리지 않는지(학습 vs 시험) 대조 필요.
- **새 전체화면 흐름 추가 시 `_EXAM_SCREENS`·`_EXAM_QUIT_FNS` 등록 체크리스트** —
  이 두 곳에 wake lock · SW reload 지연 · 뒤로가기 저장 3가지가 묶여 있음. 신규 화면
  누락 시 셋 다 조용히 안 걸림. 특히 **터치 없이 듣고 말하는 화면은 wake lock 필수**.
- **자동 새로고침은 사용 중 금지, 앱 복귀 시 적용** — 배포가 잦으면 "사용 중 새로고침"
  이 그대로 팅김 체감이 됨. `visibilitychange` hidden→visible 에서만 적용하면 배포
  빈도와 무관하게 흐름이 안 끊김.
- **카톡으로 보내는 주소는 "누르지 말고 직접 입력" 안내 필수** — 카톡 링크를 탭하면
  카톡 인앱 브라우저로 열려 음성 인식이 또 막힘. 브라우저 교체 안내가 카톡 링크
  하나로 무효화됨.
- **클라우드 루틴은 로컬 전용 자격증명이 필요한 작업에 못 씀** — admin SDK 키처럼
  git 에 없는 로컬 파일에 의존하면 Windows 작업 스케줄러 등 로컬 실행이 정답.
  키를 클라우드로 옮기는 건 개인정보 접근 범위 결정이라 사용자 승인 사항.

### 파일 / SW 캐시 (2026-09-22)
- `public/js/app.js`: `_VP_TRANSIENT_SR` + `_vpShowSrIssue` 분기 / `_EXAM_SCREENS`·
  `_EXAM_QUIT_FNS` vocabPractice 추가 / SW reload visibilitychange 방식 /
  삼성 인터넷 경고 / 설치 메뉴 노출 규칙 · alert 3곳 모달 교체
- `public/admin/js/app.js`: `warn:samsung` 라벨
- `public/_app.html`: ⋮ 버튼 안내 점(`ddInstallDot`)
- SW 캐시: `kunsori-v816` → `kunsori-v818`
- 신규 진단 (untracked): `check-student-crash.js` / `scan-speaklog-crash.js` /
  `daily-error-report.js` + `daily-error-report.bat`
- Windows 작업: `ReadAloud-DailyErrorReport` (매일 08:45)

**다음 세션 후보**:
1. **일일 리포트로 v817·v818 효과 확인** — 학습 강제퇴장이 실제로 줄었는지 (내일 아침 첫 보고)
2. 카톡 안내 발송 후 `block:no-sr`·삼성 인터넷 사용자 감소 추적
3. 마이크 권한 거부 학생 개별 조치 (박서희·이성민·범지원·김다윤 등 15건)
4. iOS `sr-hang` 완화 — 현재 재시도 안내뿐. WebKit 버그라 근본 해결 불가
5. 듣고 선택하기 실사용 피드백 / Phase 5 출시 준비 (변동 없음)
